...
export function AccordionVariantGrouped({
    ...
    const dv = resolveDefaultValue(type, items, defaultValue);
    return (
        <View style={[styles.groupedOuter, { borderColor: theme.border, backgroundColor: theme.background }]}>
            <Accordion type={type} defaultValue={dv} {...rest}>
    ...
    return (
        <Accordion type={type} defaultValue={dv} {...rest}>
            {items.map((item, index) => {
                ...
                return (
                    <AccordionItem key={item.value} value={item.value}>
                        <LeadIconTrigger title={item.title} Icon={Icon} />
                        <AccordionContent>
                            <FaqContent text={item.description} />
                        </AccordionContent>
                    </AccordionItem>
        ...
        <AccordionTrigger asChild>
            <Pressable style={({ pressed }) => [styles.triggerRow, pressed && styles.pressed]}>
                <View style={styles.leadLeft}>
                    ...
                    <View>
                        <Text style={[styles.titleMedium, { color: theme.foreground }]}>{title}</Text>
                        <Text style={{ fontSize: 12, color: theme.mutedForeground, marginTop: 2 }}>{subtitle}</Text>
                    </View>
                </View>
                <Text style={[styles.plusMinus, { color: theme.foreground }]}>{open ? '−' : '+'}</Text>
            </Pressable>
            ...
            <Accordion type={type} defaultValue={dv} {...rest}>
                {items.map((item, index) => {
                    ...
                    return (
                        ...
                        >
                            <SubtitledTrigger title={item.title} subtitle={item.subtitle} Icon={Icon} />
                            <AccordionContent>
                                <FaqContent text={item.description} />
                            </AccordionContent>
                        </AccordionItem>
