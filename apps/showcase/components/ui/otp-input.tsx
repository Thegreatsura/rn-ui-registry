...
function OTPInput({ value = "", onValueChange, maxLength = 6, disabled, style, ...props }: OTPInputProps) {
  ...
  return (
    <View style={[styles.container, style]} {...props}>
      <Pressable accessibilityRole="button" disabled={disabled} onPress={handlePress} style={styles.cellsContainer}>
        {cells.map((digit, index) => {
          ...
          return (
            ...
            >
              <Text style={[styles.cellText, { color: theme.foreground }]}>{digit}</Text>
              {isActive ? <View style={[styles.cursor, { backgroundColor: theme.primary }]} /> : null}
            </View>
          );
        })}
      </Pressable>
...
const styles = StyleSheet.create({
  ...
  cellsContainer: { flexDirection: "row", gap: 10 },
  cell: {
    width: 40,
    height: 48,
    borderRadius: 6,
  ...
  cellText: { fontSize: 20, fontWeight: "600" },
  cursor: { position: "absolute", bottom: 10, width: 16, height: 2, borderRadius: 1 },
